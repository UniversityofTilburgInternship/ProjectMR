...
namespace Casanova.Prelude
{
  ...
  public static class MyExtensions
  {
    ...
    public static List<T> Tail<T>(this List<T> list) { return list.Skip(1).ToList(); }
    ...
    public static int Length<T>(this IEnumerable<T> list) { return list.ToList().Count; }
  }
  public class Cons<T> : IEnumerable<T>
  {
    public class Enumerator : IEnumerator<T>
    {
      ...
      object IEnumerator.Current { get { throw new Exception("Empty sequence has no elements"); } }
  ...
  {
    T elem;
    public Just(T elem) { this.elem = elem; IsSome = true; }
